<template>
  <div class="room" v-if="room">
    <div
      ...
      :style="{
        backgroundImage: `url(${
          currentRoom.situations?.[0]
            ? imgFolder + currentRoom.situations?.[0].background
            : ''
        })`,
      }"
    >
      <img
        :src="
          currentRoom.situations?.[0]
            ? imgFolder + currentRoom.situations[0].sprite
            : ''
        "
        alt="sprite"
        class="sprite"
...
export default {
  ...
  setup() {
    ...
    const {question, responses, generateQuestion, loading, error} =
      useQuestionStore();
